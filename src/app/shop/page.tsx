...
export default function ShopPage() {
  ...
  const [desktopShowFilters, setDesktopShowFilters] = useState(false); // Changed to false - hide by default on desktop
  ...
  const toggleFilters = () => {
    if (isMobileView) {
      ...
      // When toggling on desktop mode, ensure we're properly showing/hiding
      setDesktopShowFilters(!desktopShowFilters);
    }
  };

  // Helper to close filters in mobile view only
  const closeMobileFilters = () => {
    // Before closing filters in mobile, ensure we preserve the desktop state
    // This is needed because we're handling a transition between views
    setMobileShowFilters(false);
    
    // Don't sync desktop state when filters are explicitly closed in mobile view
    // This allows desktop view to maintain its own state
  };
  ...
  // Check if mobile view on component mount and on resize
  useEffect(() => {
    const checkIfMobile = () => {
      const wasMobile = isMobileView;
      const isMobile = window.innerWidth < 1024;
      
      // If the view state is changing
      if (wasMobile !== isMobile) {
        ...
        if (wasMobile && !isMobile) {
          // Desktop filters should maintain their own state
          // No need to sync with mobile state here
        }
        // If transitioning from desktop to mobile view 
        else if (!wasMobile && isMobile) {
          // Mobile filters should maintain their own state
          // No need to sync with desktop state here
        }
      } else {
        // Just update the view state if no transition is happening
        setIsMobileView(isMobile);
      }
    };
      ...
      <div className={commonLayoutStyles.mainContent}>
        <div className={commonLayoutStyles.section}>
          ...
          <div className="flex justify-between items-center mb-6 mt-4">
            ...
            {/* Mobile filter toggle button - smaller and neutral color */}
            {isMobileView && (
              <button
          ...
          <div className="flex flex-col lg:flex-row gap-6 relative">
            ...
            {/* Filters Sidebar - Made to work better on mobile */}
            <motion.aside 
            ...
            >
              ...
              <div className="sticky top-0 z-10 bg-white p-4 lg:p-0 border-b border-gray-100 flex justify-between items-center mb-6">
                ...
                <div className="p-4 lg:p-0">
                  ...
                  {/* Sort By - with improved radio button styling and fixed clickability */}
                  <div className="mb-6">
                    <div 
                      className="flex justify-between items-center mb-3 cursor-pointer"
            ...
            <main className={`${showFilters && !isMobileView ? 'lg:w-3/4' : 'w-full'}`}>
              ...
              {/* Category Filter Buttons - Improved for mobile */}
              {filteredProducts.length > 0 ? (
                ...
                <div className="mb-6">
                  <div className="py-3 mb-3 border-b border-gray-100">
                    <p className="text-gray-600">
                      No robots found
                      {selectedCategories.length > 0 && 
                        ` in "${selectedCategories.join(', ')}" ${selectedCategories.length === 1 ? 'category' : 'categories'}`}
              ...
              {/* Products Grid - Improved mobile spacing */}
              {filteredProducts.length > 0 ? (
              ...
              {filteredProducts.length > productsPerPage && (
                <div className="mt-10 flex justify-center">
                  <nav className="inline-flex flex-wrap justify-center rounded-md shadow-sm -space-x-px">
                    <button
                      onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} 
                      disabled={currentPage === 1}
                      className={`px-2 sm:px-3 py-2 rounded-l-lg font-medium text-sm sm:text-base ${
                        currentPage === 1 
                    ...
                    {isMobileView ? (
                      ...
                      <div className="flex items-center px-4 py-2 bg-white border border-gray-200">
                        <span className="text-sm">
                      ...
                      Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                        ...
                        return (
                          <button 
                            ...
                            className={`w-8 sm:w-10 h-8 sm:h-10 text-sm sm:text-base ${
                              currentPage === pageNum 
                    ...
                    <button 
                      ...
                      className={`px-2 sm:px-3 py-2 rounded-r-lg font-medium text-sm sm:text-base ${
